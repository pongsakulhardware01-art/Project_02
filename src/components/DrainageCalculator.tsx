...
import { Hammer, Sparkles, Scale, Info, Copy, Check, Truck } from "lucide-react";
...
export default function DrainageCalculator({ 
  ...
  const handleCopyPrice = (amount: number) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 flex flex-col gap-6 justify-between">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-neutral-200/70 flex flex-col items-center justify-center">
            <span className="text-xs font-bold text-neutral-400 uppercase tracking-widest mb-4">โมเดลลายเส้นโครงสร้างโครงร่าง (2D Architectural Draft)</span>
          ...
          <div className="bg-gradient-to-br from-neutral-900 via-[#991B1B] to-[#7F1D1D] text-white rounded-2xl p-6 shadow-md flex flex-col justify-between min-h-[350px] border border-neutral-800">
            <div>
              ...
              <div className="mt-4 pb-3 border-b border-white/15">
                <span className="font-bold text-lg leading-snug tracking-tight block text-white">
                  {activeDetails.label}
                </span>
                <span className="text-white/60 text-xs block mt-0.5 font-mono">
                  รหัสอ้างอิง: {activeDetails.priceKey}
                </span>
              </div>
            ...
            <div className="border-t border-white/15 pt-4 mt-4 space-y-2.5 font-light text-neutral-200 text-sm">
              <div className="flex justify-between items-center">
                <span>ราคาฐาน (บาท/หน่วย):</span>
                <strong className="text-white font-semibold font-mono">฿{fmt(calcPricePerUnit)}</strong>
              </div>
              <div className="flex justify-between items-center">
                <span>จำนวนจำเพาะ:</span>
                <strong className="text-white font-semibold font-mono">{calcQty} ชิ้น/ท่อน</strong>
              </div>
              <div className="flex justify-between items-center">
                <span>ราคาสุทธิก่อนภาษี (VAT):</span>
                <strong className="text-white font-semibold font-mono">฿{fmt(priceBeforeVat)}</strong>
              </div>
              ...
              {calcAdditionalPrice > 0 && (
                <div className="flex justify-between items-center text-amber-300">
                  ...
                  <strong className="font-mono font-bold">฿{fmt(calcAdditionalPrice)}</strong>
                </div>
              )}
              <div className="flex justify-between items-center pt-2 border-t border-white/10">
                <span>น้ำหนักขนส่งรวม:</span>
