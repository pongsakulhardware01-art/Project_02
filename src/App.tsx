          ...
          <AnimatePresence mode="wait">
            {currentScreen === "menu" ? (
              <motion.div
                ...
                className="space-y-6"
              >
                {/* 1. HERO/BANNER FOR WORKSPACE STYLE */}
                <div className="relative overflow-hidden bg-gradient-to-r from-neutral-900 via-[#991B1B] to-[#7F1D1D] text-white rounded-3xl p-6 md:p-8 shadow-sm border border-neutral-800">
                  ...
                  <div className="space-y-4 relative z-10 max-w-3xl">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="inline-flex items-center gap-1.5 bg-white/10 text-neutral-200 text-xs font-semibold px-2.5 py-1 rounded-full border border-white/15">
                        <Factory size={12} className="text-amber-300" />
                        โรงงาน: {activeSupplier.name}
                      </span>
                    </div>
                    ...
                    <div className="pt-2 flex flex-wrap items-center gap-2.5">
                      <button
                        ...
                        className="py-2 px-3.5 bg-white text-neutral-900 hover:bg-neutral-100 active:scale-98 rounded-xl font-bold text-xs shadow-xs transition flex items-center gap-1.5 cursor-pointer"
                      >
                        <Calculator size={14} className="text-[#C62828]" />
                        <span>คำนวณราคาด่วน</span>
                      </button>
                      <button
                        onClick={() => handleScreenChange("scan")}
                        className="py-2 px-3.5 bg-amber-500/30 hover:bg-amber-500/45 text-amber-200 border border-amber-400/40 active:scale-98 rounded-xl font-bold text-xs shadow-xs transition flex items-center gap-1.5 cursor-pointer"
                      >
                      ...
                      <button
                        onClick={() => handleScreenChange("weight")}
                        className="py-2 px-3.5 bg-white/15 hover:bg-white/25 text-white active:scale-98 rounded-xl font-bold text-xs border border-white/20 shadow-xs transition flex items-center gap-1.5 cursor-pointer"
                      >
                        <Truck size={14} className="text-amber-300" />
                        <span>ตรวจสอบระวางรถ</span>
                      </button>
                      <button
                        onClick={() => setSupplierDropdownOpen(true)}
                        className="py-2 px-3.5 bg-white/10 hover:bg-white/20 text-neutral-200 active:scale-98 rounded-xl font-medium text-xs border border-white/10 transition flex items-center gap-1.5 cursor-pointer"
                      >
                        <Building2 size={14} />
                        <span>สลับโรงงาน ({activeSupplier.name})</span>
                      </button>
                    </div>
                  </div>
                </div>

                {/* Live Cargo Dispatch Preview Bar (if items in cargo) */}
                {weightItems.length > 0 && (
                  <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200/80 rounded-2xl p-4 md:p-5 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
                      ...
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-neutral-800 text-sm md:text-base">
                            มีสินค้าค้างอยู่ในระวางขนส่ง {weightItems.length} รายการ
                          </span>
                          <span className="text-[10px] bg-amber-200 text-amber-900 font-extrabold px-2 py-0.5 rounded-full">
                            น้ำหนักรวม {(totalWeightKg / 1000).toFixed(2)} ตัน
                          </span>
                        </div>
                        <p className="text-xs text-neutral-600 font-light">
                          รถที่แนะนำ: <strong className="text-neutral-800 font-bold">{recommendedTruckText || "ยังไม่ระบุ"}</strong>
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <button
                        onClick={() => handleScreenChange("weight")}
                        className="py-2 px-4 bg-amber-600 hover:bg-amber-700 active:scale-98 text-white rounded-xl font-bold text-xs transition flex items-center gap-1.5 shadow-xs cursor-pointer"
                      >
                        <span>เปิดระบบจัดสรรรถขนส่ง</span>
                        <ChevronRight size={14} />
                      </button>
                      <button
                        onClick={() => setWeightItems([])}
                        className="py-2 px-3 bg-white hover:bg-neutral-100 text-neutral-500 hover:text-red-600 rounded-xl font-medium text-xs border border-neutral-200 transition cursor-pointer"
                        title="ล้างรายการในระวางรถทั้งหมด"
                      >
                        ล้างรายการ
                      </button>
                    </div>
